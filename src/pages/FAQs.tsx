...
const FAQs = () => {
  ...
  const loadAnsweredQuestions = async () => {
    try {
      ...
      setQuestions(data || []);
      setFilteredQuestions(data || []); // Set initial filtered questions
    } catch (error) {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim() === '') {
      setFilteredQuestions(questions); // Reset to all questions if search is empty
    } else {
      const filtered = questions.filter((q) => 
        q.question.toLowerCase().includes(value) || 
        (q.answer && q.answer.toLowerCase().includes(value)) // Check if answer exists before calling toLowerCase()
      );      
