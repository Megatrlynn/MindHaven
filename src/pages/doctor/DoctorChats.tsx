...
const DoctorChats: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socket.on("call-declined", () => {
      console.log("ðŸ“´ Call declined by patient");
      setIsCalling(false);
    ...
    socket.on("end-call", () => {
      console.log("ðŸ”š Call ended by patient");
      setIsCalling(false);
  ...
  const startCall = async (patientId: string) => {
    if (!doctorId) {
      console.error("âŒ Doctor ID not available");
      return;
    }
  
    if (!connectedPatients.includes(patientId)) {
      console.error("âŒ Cannot call: Patient is not connected!");
    ...
    try {
      ...
      if (!localStream || localStream.getAudioTracks().length === 0) {
        console.error("âŒ No audio tracks found!");
      ...
      console.error("âŒ Error starting call:", error);
    ...
    const handleIceCandidate = async (data: any) => {
      try {
        ...
        if (!candidateData.candidate || (candidateData.sdpMid === null && candidateData.sdpMLineIndex === null)) {
          console.error("âŒ Invalid ICE candidate received (missing sdpMid and sdpMLineIndex)", candidateData);
          return;
        }
        ...
        console.error("âŒ Error adding ICE candidate:", error, "Candidate data:", data);
      }
    };
  ...
  const handleOffer = async (offer: RTCSessionDescriptionInit, from: string) => {
    try {
      ...
      console.error("âŒ Error handling offer:", error);
    }
  };
    ...
    <div className="p-6 md:p-8">
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-bold text-[var(--mh-text)]">Patient Connections</h2>
          <p className="mt-1 text-sm text-[var(--mh-text-muted)]">Accept requests, start chats, and place voice calls from one workspace.</p>
        </div>

        <div className="inline-flex items-center gap-2 rounded-xl border border-[var(--mh-border)] bg-[var(--mh-surface)] px-4 py-2 text-sm text-[var(--mh-text-muted)] shadow-sm">
      ...
      <div className="mb-4">
        <label className="relative block max-w-xl">
          ...
          <input
            ...
            className="w-full rounded-xl border border-[var(--mh-border)] bg-[var(--mh-surface)] py-2 pl-10 pr-3 text-sm text-[var(--mh-text)] placeholder:text-[var(--mh-text-muted)] focus:border-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-200"
          />
        </label>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
        {connections.map((connection) => (
          <div key={connection.id} className="surface-card p-6 transition-transform hover:-translate-y-0.5 hover:shadow-md">
            <div className="flex items-center mb-5">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[var(--mh-surface-soft)] shadow-sm">
                <UserCircle className="h-12 w-12 text-[var(--mh-text-muted)]" />
              </div>
              <div className="ml-4">
                <h3 className="text-lg font-semibold text-[var(--mh-text)]">
                  Patient #{connection.patient_id.slice(0, 8)}
                </h3>
                {connection.patient_profile && (
                  <div className="text-sm text-[var(--mh-text-muted)]">
        ...
        {connections.length === 0 && (
          <div className="col-span-full rounded-2xl border border-dashed border-[var(--mh-border)] bg-[var(--mh-surface-soft)] py-10 text-center text-[var(--mh-text-muted)]">
      ...
      {isCalling && selectedPatientId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="surface-card w-full max-w-sm p-5 text-center">
            <h2 className="mb-3 text-lg font-bold text-[var(--mh-text)]">{callStatus}</h2>
            {remoteStream ? (
              <audio ref={remoteAudioRef} autoPlay playsInline controls />
            ) : (
              <p className="text-[var(--mh-text-muted)]">Waiting for audio...</p>
            )}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
    <div className="surface-card flex h-[80vh] w-full max-w-4xl flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-[var(--mh-border)] bg-[var(--mh-surface-soft)] p-5 text-[var(--mh-text)]">
        <div>
        ...
        <button
          ...
          className="rounded-lg p-2 text-[var(--mh-text-muted)] transition-colors hover:bg-[var(--mh-surface)] hover:text-[var(--mh-text)]"
        >
          <X className="w-6 h-6" />
        </button>
      </div>

      <div className="flex-1 overflow-y-auto bg-[var(--mh-surface-soft)] p-5 space-y-5">
        {loadingChat ? (
              ...
              <div key={msg.id} className={`flex flex-col ${isDoctor ? "items-end" : "items-start"} mb-1`}>
                <div
                  className={`max-w-[75%] rounded-xl p-3 shadow-md ${
                    isDoctor ? "bg-cyan-700 text-white" : "bg-[var(--mh-surface)] text-[var(--mh-text)] border border-[var(--mh-border)]"}`}
                >
      ...
      <div className="border-t border-[var(--mh-border)] bg-[var(--mh-surface)] p-5">
        <div className="flex items-center space-x-4">
          <input
            ...
            className="flex-grow rounded-lg border border-[var(--mh-border)] bg-[var(--mh-surface)] p-3 text-[var(--mh-text)] placeholder:text-[var(--mh-text-muted)] shadow-sm transition focus:border-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-200"
            onKeyPress={(e) => {
